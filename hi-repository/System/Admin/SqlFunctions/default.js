/**function setQueryOffsetLimit(query, offset, limit,context) {
	...
	var databaseNameToSplitJoin = contextObj.openQuote+contextObj.databaseName+contextObj.closeQuote+".";
	query=query.split(databaseNameToSplitJoin).join("");
...
function checkDistinctOnFirstColumn(obj){
	if(obj.formData.functions.aggregate){
		var functionName = obj.formData.functions.aggregate[0].function;
		return(functionName.startsWith('distinct'))
	}
	return false;
}
function isDistinctResults(obj){
	return obj.distinctResults;
}
function applyDistinctOnOuter(obj){
	return (isDistinctResults(obj) && isAggrigate(obj))
}
function isAggrigate(obj){
	
	if(isSearchingResult(obj)){
		var singleColumn = obj.formData.columns[0];
		if(singleColumn.aggregate){
			return singleColumn.aggregate
		}else{
			return false
		}
	}else{return false}
}
function isSearchingResult(obj){
	if(obj.formData.columns.length ===1){
		return true;
	}else if(obj.formData.columns.length ===2){
		
		return(obj.formData.columns[0].column === obj.formData.columns[1].column)
	}
	return false;
}
function removeRank(obj){
	if(obj.formData.columns.length ===1 && !isDistinctResults(obj)){
		var singleColumn = obj.formData.columns[0];
		if(singleColumn.aggregate){
			return singleColumn.aggregate
		}else{
...
function isSingleColumn(obj){
	return(obj.formData.columns.length ===1)
}
function setQueryOffsetLimit(query, offset, limit, context) {
   ...
   // return query;
	...
	//return applyDistinctOnOuter(obj);
   ...
   // return fromClause;
	...
	whereWithoutDbName =whereWithoutDbName.replaceAll("'true'","true").replaceAll("'false'","false");
	whereWithoutDbName =whereWithoutDbName.replaceAll("'TRUE'","TRUE").replaceAll("'FALSE'","FALSE");
 var  fromClauseWithoutJoin=fromClause;
 if(fromClause.indexOf("\n\t",5)>-1){
	 
 fromClauseWithoutJoin=fromClause.substring(0,fromClause.indexOf("\n",5));
 }
   if((fromClauseWithoutJoin.match(/from/g) || []).length ==1 && fromClauseWithoutJoin.split(".").length >= 3 && fromClauseWithoutJoin.indexOf('')){
...
//return aliasesText;
	...
	if(applyDistinctOnOuter(obj) || checkDistinctOnFirstColumn(obj)){
		selectWithoutDbName = selectWithoutDbName.replace("distinct","");
		aliasesJoined = "distinct  "+aliasesJoined;
	}
    if (orderByWithoutDbName){
		if(removeRank(obj)){
			requiredQueryForSpark = selectKeyword + '' + aliasesJoined + ' from (' + selectWithoutDbName + ',' + appliedRank + ' over ( ' + orderByWithoutDbName + ')';
		}else{
		...
		if(removeRank(obj)){
			 requiredQueryForSpark = selectKeyword + '' + aliasesJoined + ' from (' + selectWithoutDbName + ',' + appliedRank + ' over (ORDER BY 1)';
		}else{
